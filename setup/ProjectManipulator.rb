...
module Pod
  ...
  class ProjectManipulator
    ...
    def run
      @string_replacements = {
        "PROJECT_OWNER" => @configurator.user_name,#此处有一个注意点，当git配置为中文名时，会报错
        "TODAYS_DATE" => @configurator.date,#修改为创建日期
        "TODAYS_YEAR" => @configurator.year,#修改为创建年
        "PROJECT" => @configurator.pod_name,#修改为你的lib名
        "CPD" => @prefix#将CPD修改为你自己定义的前缀
      }
